...
### Changed
- **`whs doctor` skips auth check by default**: The slow Claude auth check (shells out to `claude --print`) is now opt-in via `--auth` flag. All other checks run as before

## [0.14.1] - 2026-02-15

### Added
- **Minor review feedback routing**: Quality review now routes PASS-with-minors to implementation for a single triage pass. Implementation agent reviews each minor item (fix worthwhile ones, skip nitpicky ones) and includes a `MINOR_FEEDBACK_REVIEWED` marker to prevent infinite loops
...
### Changed
- **CI polling no longer floods the log stream**: Removed per-tick "Checking CI" and "CI still running" log messages since the TUI bottom panel now shows CI-pending status. State-change logs (passed, failed, conflicts) are kept
...
### Fixed
- **CI check phase was silent**: `checkPendingCI` produced no output when polling PRs or when CI was still running. Now logs which PRs are being checked and shows status for pending checks
